...
from utils.config import NEWAPI_SITES, AppConfig
...
class PlatformManager:
    ...
    async def _run_all_linuxdo(self) -> list[CheckinResult]:
        """运行 LinuxDO 统一账号签到（一个账号签到多个站点 + 浏览帖子）"""
        ...
        for i, account in enumerate(self.config.linuxdo_accounts):
            ...
            # 1. 先浏览 LinuxDO 帖子（增加在线时间）
            if account.browse_linuxdo:
                logger.info("  浏览 LinuxDO 帖子...")
                browse_adapter = LinuxDOAdapter(
                ...
                try:
                    browse_result = await browse_adapter.run()
